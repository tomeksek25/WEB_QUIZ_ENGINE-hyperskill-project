...
Short description: 
REST API using HTTP Basic Auth, where you can create Quizzes and solve them. There is User repository (you can register), Quiz repository and Completion repository. GET posiibilities: Get all quizzes, Get one quiz, Get your quizzes completions with data/time (all Get are with pagination). POST: create quizz (with conditions), solve quiz, register. DELETE - deletes quiz. 
